...
import Windows from "./Contact_Windows";
import flecha from "../../../icon/sort-alt.svg";
import Header from "./Header";
import {
  Search_Contact,
  Search_Contact_Canal,
  Consult_RPC,
  Company_CRUD,
  Search_Contacts,
  Delete_A_Contact,
...
function Contact(props) {
  ...
  async function inicio() {
    const Result = await Search_Contact();
    llenarpagina(Result);
  }
  ...
  async function llenarpagina(Result) {
    ...
    const ResultadoRegion = await Consult_RPC(BaseBody);
  ...
  async function MostrarPreferido(ID) {
    ...
    const Result = await Search_Contact_Canal(BaseBody);
    if (Result.estado === "Correcto") {
      if (Result.msj.length !== 0) {
  ...
  async function eliminar(e) {
    ...
    let Respuesta = await Delete_A_Contact(base);
  ...
  async function buscar(e) {
    ...
    let Resultado = await Search_Contacts(base);
      ...
      <main>
        ...
        <section className="Superior">
          ...
          <div className="contenedor_botones">
            {VetanaContacto !== "" ? (
              <Windows ID={IDActualizar} Cerrar={Cierre} />
            ) : (
              ""
            )}
