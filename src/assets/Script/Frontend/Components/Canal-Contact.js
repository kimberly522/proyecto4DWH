...
import { Insert_Contact, Insert_Canal } from "../consult";
  ...
  async function GuardarContacto(e) {
    ...
    if (
      ...
      if (canales.length === 0) {
        ...
        const result = await Insert_Contact(BaseBody);
        ...
        async function GuardarContactoCanal(ID) {
          ...
          Coleccion.forEach(async (elemento) => {
            ...
            const resultado = await Insert_Canal(Canales);
